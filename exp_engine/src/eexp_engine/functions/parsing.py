    ...
    for assembled_wf_data in assembled_wfs_data:
        ...
        for task in wf.tasks:
            if task.name in assembled_wf_data["tasks"].keys():
                ...
                if "pre_script" in task_data:
                    logger.info(f"Changing pre_script of task '{task.name}' to '{task_data['pre_script']}'")
                    task.add_pre_script(task_data["pre_script"])
...
def parse_task(folder_path):
    ...
    for component in workflow_model.component:
        ...
        for e in component.elements:
            ...
            if e.__class__.__name__ == "PreScript":
                if e.pre_script:
                    prescript_file_path = os.path.join(CONFIG.TASK_LIBRARY_PATH, e.pre_script)
                    parsed_data["pre_script"] = prescript_file_path
            ...
            for e in component.elements:
                ...
                if e.__class__.__name__ == "ConfigureTask":
                    ...
                    if e.filename:
                        ...
                        task.add_pre_script(parsed_data.get("pre_script"))
            ...
            while configurations:
                for config in component.tasks:
                    ...
                    if config.workflow:
                        ...
                        assembled_workflow_task["pre_script"] = parsed_data.get("pre_script")
